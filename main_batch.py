...
def test_prf(pred, labels):#pred和labels中的元素只有1和0
    """
    4. log and return prf scores
    :return:
    """
    total = len(labels)
    pred_right = [0, 0]#记录pred和labels中0和1在相同位置出现的次数
    pred_all = [0, 0]#记录pred中0和1分别出现的次数
    gold = [0, 0]#记录labels中0和1分别出现的次数
    ...
    ''' -- for all labels -- '''
    print ("  ****** Neg|Neu|Pos ******")
    accuracy = 1.0 * sum(pred_right) / total#预测正确的元素数和比上长度（预测正确精度）
    p, r, f1 = yutils.cal_prf(pred_all, pred_right, gold, formation=False)#预测
...
def test(model, dataset, args, data_part="test"):
    ...
    tit = time.time() - tic
    print ("  Predicting {:d} examples using {:5.4f} seconds".format(len(test_set), tit))#记录时间
...
def train(model, training_data, args, optimizer, criterion):#进行训练
    model.train()
    ...
    probs = model(sentences_, sentences_seqlen_, sentences_mask_)
    loss = criterion(probs.view(len(labels_), -1), labels_)#计算loss

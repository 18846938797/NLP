...
def read_emb_idx(filename):
    ...
    with open(filename, 'r') as f:
        ...
        embeddings.insert(0, np.zeros(emb_dim))  # _padding 在embedding的开头添加长度为embedding【0】的0向量
        embeddings.insert(1, np.random.random(emb_dim))  # _unk    然后添加相同长度的随机向量

        embeddings = np.asarray(embeddings, dtype=np.float32)
        embeddings = embeddings.reshape(len(embeddings), emb_dim)'''改变数组的形状'''
...
def processing(args):
    ...
    yutils.dict2pickle(datasets, output_dir + "/features_glove.pkl")#序列化对象，并将结果数据流写入到文件对象中。
